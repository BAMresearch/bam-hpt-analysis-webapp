...
2. Define cycles. **Suggest cycles is one click.** It cuts the file **drop to drop** on uncorrected electric power: a cycle starts at the **first leave of the long heating plateau**. If power hardly moves, Start follows T_supply and heating. The later collapse inside a defrost is not the start. Short blinks that return to the plateau are ignored. On a defrost file (test condition A/B/E/F) a suggested window in which no defrost actually happened — supply − return never stays below 0.2 K for 60 s — is left out; a C/D or unlabelled file, a file without those temperature columns, and the case where that would leave nothing to suggest all keep the full list. Every suggested start and end stays editable, and you can still click the plot to add a cycle by hand.
